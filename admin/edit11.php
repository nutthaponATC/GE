...
<body>
	...
	<div style="width:80%; height:100%; left:20%; float:left;  position:relative;">
		<div id="info1" class="container">
			<form action="edit_process11.php" method="post" enctype="multipart/form-data">
				<?php 
				...
				<h3>เพิ่มหนังสือนอกเวลา</h3>
				<br>
				<h4>ชื่อหนังสือนอกเวลา</h4><input class="form-control" type="text" name="header" value="<?php echo $data['title']; ?>">
				...
				<a href="remove11.php?id_public=<?php echo $data['id_public']; ?>"><input id="bt" class="btn btn-default" style="margin-top:10px; background-color:b45564; color:white;" type="button" name="delete" value="ลบ"></a>
			</form>
			<table id="example" class="display" style="font-size: 15px; padding-top:30px;" cellspacing="0" width="100%">
		        ...
		        <tbody>
		        	...
		        	while ($data = mysql_fetch_array($query)) {
		        		echo "
		        		<tr style='cursor:pointer;' data-href='edit11.php?id_public=".$data['id_public']."'>
		        			<td>".$i."</td>
			                <td>".$data['title']."</td>
			                <td>".$data['file']."</td>
			                <td><center><a href='edit11.php?id_public=".$data['id_public']."'><i class='fa fa-cog' style='color:#b45564; font-size:30px;' aria-hidden='true'></i></a></center></td>
				            <td><center><a href='remove11.php?id_public=".$data['id_public']."'><i class='fa fa-times' style='color:#b45564; font-size:30px;' aria-hidden='true'></a></i></center></td>
			            </tr>";
			            $i++;
		        	}
		        	 ?>
